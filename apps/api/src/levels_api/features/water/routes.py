...
def _requested_date(timezone: str | None = None) -> date:
    ...
    if value is None:
        resolved_timezone = timezone or require_profile(get_db(), current_user_id()).timezone
...
def get_today_water() -> tuple[Response, int]:
    ...
    return jsonify(water_day(get_db(), user_id, _requested_date())), 200
...
def post_water() -> tuple[Response, int]:
    ...
    with transaction() as session:
        result = add_water(session, user_id, write, request.headers.get("Idempotency-Key"))
...
def post_undo_water() -> tuple[Response, int]:
    ...
    with transaction() as session:
        ...
        result = undo_latest(session, user_id, _requested_date(profile.timezone))
